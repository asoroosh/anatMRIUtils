...
# Copy the image from the processed directory to unprocessed
cp ${InputDir} ${OutputDir}/${ImageName}.nii.gz

# decompress the image, CAT12 only works with decompressed images
gunzip -d ${OutputDir}/${ImageName}.nii.gz

# Jump into the process directory and run CAT12 in their folder -- this is a bit stupid but that is how it is with CAT12
cd ${OutputDir}

# And now the operations
${HOME}/AUX/spm12/toolbox/cat12/cat_batch_cat.sh ${ImageName}.nii
