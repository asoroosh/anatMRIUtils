...
JobName=${StudyID}_${DirSuffix}_${ReRunDirSuffix}_${NUMJB}
...
############# Find the QC Status
TheReRunTxtFile="/data/ms/processed/mri/QC/${StudyID}/${DirSuffix}_norm_RAS/${StudyID}_${DirSuffix}_norm_RAS_Report.txt"

if [ ! -f \$TheReRunTxtFile ]; then
	echo "The QC file does not exists: \$TheReRunTxtFile :: does not exists."
	exit 1
fi

echo "Checking the status of: \${SubIDVar}_\${SesIDVar} :"

QC_Status=\$(cat \$TheReRunTxtFile | grep "\${SubIDVar}_\${SesIDVar}" | awk '{print \$2}')

echo \$QC_Status

if [ \$QC_Status == 1 ]; then
	echo "The image is just fine, QC Status: \$QC_Status, so we skip!"
	exit 1
fi
