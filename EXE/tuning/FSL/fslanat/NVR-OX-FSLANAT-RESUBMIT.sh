...
if [ -d \${OutputDir} ]; then
	...
	if [ $REFERESH == 1 ]; then
		...
		echo "I will delete the directory and resubmit the job"
		rm -rf \${OutputDir}

	elif [ $REFERESH == 0 ]; then

#	This is super cool, but require the
		#14479

		if [ ! -z $TaskID ]; then
			JBSTAT=\$(cat ${ImgTypOpLog}/${JobName}_${TaskID}_\${SLURM_ARRAY_TASK_ID}.stat | awk '{print \$2}')
			echo "Job status of the previous run is: \$JBSTAT"

			if [ \$JBSTAT == 0 ]; then
				echo "The output exists, but failed: we'll re-run again now..."
				rm -rf \${OutputDir}
				#continue;
			elif [ \$JBSTAT == 1 ]; then
				echo "The image has already been processed successfully. We'll skip!"
				exit 1
			fi
		elif [ -z $TaskID ]; then 
