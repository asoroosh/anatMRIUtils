...
source /home/bdivdi.local/dfgtyk/NVROXBOX/SOURCE/reg/setpathinanalytics
...
--output [${antsRegOutputPrefix}SYN0,${antsRegOutputPrefix}SYN0.nii.gz]
...
#--masks [NULL,${NonLinSSTDirImg_pad}_BE${seglab}-REG${reglab}_brain_mask_dil1.nii.gz] #>> ${REG_LOG}
...
#--smoothing-sigmas ${SmthFctrs} \
#--transform SyN[0.1,3,0] \
#--metric CC[${MNIImg_FS}.nii.gz,${NonLinSSTDirImg_pad}.nii.gz,1,4] \
#--convergence [${ItrNum},1e-6,10] \
#--shrink-factors ${ShrnkFctrs} \
#--smoothing-sigmas ${SmthFctrs} >> ${REG_LOG}

#-x [${MNIImg_FS}_mask.nii.gz,${NonLinSSTDirImg_pad_mask}.nii.gz] >> ${REG_LOG}
