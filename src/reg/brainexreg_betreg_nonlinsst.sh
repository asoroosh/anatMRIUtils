...
if [ $do_bex == 1 ]; then
	...
	echo "Input: ${NonLinSSTDirImg}.nii.gz"
	echo "Output: ${NonLinSSTDirImg}_${BETREGOP}_brain.nii.gz"

	bet ${NonLinSSTDirImg} ${NonLinSSTDirImg}_${BETREGOP}_brain -R -m -S

	echo "Brain extraction done; "
	echo "check: ${NonLinSSTDirImg}_${BETREGOP}_brain.nii.gz"

	if [ ! -f ${NonLinSSTDirImg}_${BETREGOP}_brain.nii.gz ]; then
		echo "BET FAILED:: ${NonLinSSTDirImg}_${BETREGOP}_brain.nii.gz does not exist."
		exit 1
	fi
else
...
#	SesID=${SesIDList[v_cnt]}
	...
	UnprocessedImg=${UnprocDir}/sub-${SubID}/ses-${SesID}/anat/sub-${SubID}_ses-${SesID}_run-1_T1w
...
#
#
#	I used the code above to get the brain stuff -- aiming to fasten things up + use N3 FS correction 
#	But then changed it to the following as we use N4 already and we wanna avoid interpolations
