...
# When run on SLURM the original script might exit with error due to memory allocation
# I edited line 1003 for the memory allocation with variable SBATCH_MEM.
# Soroosh Afyouni, 2020
SBATCH_MEM=7500M
...
if [ "$RIGID" -eq 1 ] ;
    ...
    for IMG in $IMAGESETVARIABLE
      ...
      if [ $DOQSUB -eq 1 ] ; then
        ...
        id=`sbatch --job-name=antsrigid --export=ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH $QSUBOPTS --nodes=1 --cpus-per-task=1 --mem-per-cpu=${SBATCH_MEM} --time=4:00:00 $qscript | rev | cut -f1 -d\ | rev`
        jobIDs="$jobIDs $id"
        sleep 0.5
...
# fi
  ...
  for IMG in $IMAGESETVARIABLE
    ...
    if [ $DOQSUB -eq 1 ]; then
      ...
      echo -e "$exe" >> $qscript
      id=`sbatch --mem-per-cpu=${SBATCH_MEM} --job-name=antsdef${i} --export=ITK_GLOBAL_DEFAULT_NUMBER_OF_THREADS=1,LD_LIBRARY_PATH=$LD_LIBRARY_PATH,ANTSPATH=$ANTSPATH --nodes=1 --cpus-per-task=1 --time=4:00:00 $QSUBOPTS $qscript | rev | cut -f1 -d\ | rev`
      jobIDs="$jobIDs $id"
      sleep 0.5
    elif  [ $DOQSUB -eq 0 ] ; then
      bash $exe
